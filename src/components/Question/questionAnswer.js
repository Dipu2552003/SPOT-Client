...
const Answer = (props) => {
  ...
  var checkrightvariable = (
    <div>
      <i class="fa fa-check"></i>
    </div>
  );
  const [checkright, setcheckright] = useState(
    answer.is_verified ? checkrightvariable : ""
  ); //setcheckright never used
  // var linkanswer = "#" + answer._id;
  const [count_vote, setcount_vote] = useState(
    answer.liked_by.length - answer.unliked_by.length
  );
  const profile_url =
    "https://avatars.dicebear.com/api/gridy/" + answer.answered + ".svg";
  ...
  function addVote(vote) {
    ...
    if (jwttoken !== "") {
      ...
      axios
        .post("https://askoverflow-server.vashishth-patel.repl.co/answervote", {
          answerid: answer._id,
          vote: vote,
  ...
  const fun_verifyAnswer = async () => {
    if (
      window.confirm("Are ou sure? You are verifying this answer!!") === true
    ) {
      setverifyAnswerloading(imgforloadvote);
      axios
        .post(
          "https://askoverflow-server.vashishth-patel.repl.co/answerverify",
          {
            answerid: answer._id,
  ...
  if (jwttoken1 !== "") {
    if (userid === question_owner) {
      verifyAnswer = (
        <span className="fc-light mr2" data-toggle="modal">
          <button className="btn" onClick={fun_verifyAnswer}>
            verify
          </button>
        </span>
      );
        ...
        <div className="col-lg-11 col-md-11 col-sm-11 col-10 bg-light text-dark">
          ...
          <div class="row">
            <div className="col-8">
              ...
              <div
                class="modal fade"
                id={"editAnswer" + answer._id}
                tabindex="-1"
                role="dialog"
                aria-labelledby="editAnswerCenterTitle"
                aria-hidden="true"
              >
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="editAnswerLongTitle">
                        Edit Your Answer
                      </h5>
                      <button
                        type="button"
                    ...
                    <div class="modal-body">
                      <EditAnswer
                        currentAnswer={answer.answer_body}
                        answerId={answer._id}
                      />
                    </div>
                  </div>
            ...
            <div className="col-4">
              <div className="font-weight-light text-secondary">
                <span title="2019-03-03 16:34:02Z" className="relativetime">
                  {answer.answered_on}
                </span>
              </div>
              <div>
                <div class="user-gravatar32">
                  <img
                    ...
                    className="bar-sm"
                  />{" "}
                  &nbsp; <a href="#hello">{answer.answered}</a>
                </div>
              </div>
            </div>
          </div>
